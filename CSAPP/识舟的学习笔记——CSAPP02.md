...
早期的浮点数表示各厂商很不统一，不过电气电子工程师学会在1985年提出的IEEE754的初版标准提出之后有了较为统一的浮点数表示。后来对这些也有不同程度的修订。
...
$exp = 0, frac = 0表示\pm0$，非规格化的作用是渐进下溢（gradual underflow），让最小规格化数和 0 之间平滑过渡。
...
其中需要注意的是向偶数舍入（Nearest Even），一般情况下四舍六入，只有舍去部分恰好为一半时向偶数舍，确保舍去后最后一位为偶数（四舍六入偶成双）。

#### Closer Look at Round-To-Even

从统计学意义来讲，向偶数舍入确保了向上和向下取整的概率都是50%。

#### Floating Point Multiplication

对于乘法计算：$(-1)^{s1}\ M1\ 2^{E1}\ ×\ (-1)^{s2}\ M2\ 2^{E2}$，结果为$(-1)^s\ M\ 2^E$，其中：
...
应当注意，整型不完全等于整数，浮点型也不完全等于实数，不过二者有类似的数学特性，但不能将二者完全划等号，比如浮点数符合交换律但是不一定满足结合律。其他特性还有不满足分配律、满足单调性等。
